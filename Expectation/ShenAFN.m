...
- (void)uploadimgWithurl:(NSString *)urlStr image:(UIImage *)image fileName:(NSString *)fileName username:(NSString*)username success:(void (^)(id responseObject))success fail:(void (^)())fail
{
    ...
    [manager POST:urlStr parameters:parameter constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
         ...
         */
        [formData appendPartWithFileData:UIImageJPEGRepresentation(image,0.5) name:@"file" fileName:[NSString stringWithFormat:@"%@.jpeg",fileName] mimeType:@"image/jpeg"];
...
{
    //需要传递的参数,jason格式
    NSDictionary *parameter = @{@"pic": @"10000"};
    ...
    [manager POST:urlStr parameters:parameter constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for (int i=0; i<[imgarray count]; i++) {
            [formData appendPartWithFileData:UIImageJPEGRepresentation([imgarray objectAtIndex:i],0.5) name:[NSString stringWithFormat:@"file" ] fileName:[NSString stringWithFormat:@"%@.jpeg",name[i]] mimeType:@"image/jpeg"];
        }
    } success:^(NSURLSessionDataTask *task, id responseObject) {
        if (success) {
            //NSLog(@"success\n");
