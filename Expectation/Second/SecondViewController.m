...
#import "topCell2_5s.h"
#import "bottomCell1_5s.h"
#import "Login_5s.h"
#import "Advice_5s.h"
#import "Download_5s.h"
#import "single_5s.h"
#import "bottomCell2.h"
#import "bottomCell2_5s.h"
...
@property (strong, nonatomic)topCell2 *cellForHeader;
@property (strong, nonatomic)topCell2_5s *cellForHeader_5s;
@end

@implementation SecondViewController

static NSString * const reuseIdentifier1 = @"topCell2";
static NSString * const reuseIdentifier1_5s = @"topCell2_5s";
static NSString * const reuseIdentifier2 = @"bottomCell2";
static NSString * const reuseIdentifier2_5s = @"bottomCell2_5s";
...
- (void)viewDidLoad {
    ...
    { [self.tableView registerNib:[UINib nibWithNibName:@"topCell2_5s" bundle:nil]
           forCellReuseIdentifier:reuseIdentifier1_5s];
        [self.tableView registerNib:[UINib nibWithNibName:@"bottomCell2_5s" bundle:nil]  forCellReuseIdentifier:reuseIdentifier2_5s];
        cell_height=55;
        
    }
    else
    {
        [self.tableView registerNib:[UINib nibWithNibName:@"topCell2" bundle:nil]  forCellReuseIdentifier:reuseIdentifier1];
        [self.tableView registerNib:[UINib nibWithNibName:@"bottomCell2" bundle:nil]  forCellReuseIdentifier:reuseIdentifier2];
        cell_height=70;
    }
 ...
 //   [self.tableView registerClass:[topCell2 class] forCellReuseIdentifier:reuseIdentifier1];
...
-(void)updateInterval
{
    if (_detailArray.count > 0)
    {
        for( int i=1;i<count;i++)
        {
            ...
            if(![model.interval isEqual:interval])
            {
                [self updateIntervalData: [model.id intValue] n:i iftop:NO interval:interval];
                [self initTableView];
//                if([model.interval isEqual:@"0"]){
...
//                }
    ...
    if (_detailArray1.count > 0)
    {
        ...
        {
            dayModel * model = _detailArray1[i];
            NSDate * today = [NSDate date];
            NSDate *currentTime = [NSDate dateWithTimeIntervalSince1970:[model.time intValue]];
            NSString *interval = [NSString stringWithFormat:@"%ld",labs((long)[self getDaysFrom:today To:currentTime])];
            ...
            if(![model.interval isEqual:interval])
            {
                [self updateIntervalData: [model.id intValue] n:i iftop:YES interval:interval];
                [self initTableView];
//                if([model.interval isEqual:@"0"]){
//                [self updateIfpass:[model.id intValue]];
//                }
...
-(void)updateIntervalData:(int)ID n:(int)n iftop:(BOOL)iftop interval:(NSString*)interval
{
    ...
    else
    {
        ...
        //   char *errorMsg = NULL;
        sqlite3_stmt *stmt;
        NSString *update = [NSString stringWithFormat:@"update day set interval='%@' where id='%d'",interval,ID];
...
-(void)updateIfpass:(int)ID
{
    ...
    if (sqlite3_open([[self dataFilePath] UTF8String], &database)
        ...
        //    char *errorMsg = NULL;
        sqlite3_stmt *stmt;
        NSString *update = [NSString stringWithFormat:@"update day set ifpass=1 where id='%d'",ID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if([[AppDelegate deviceVersion] isEqual:@"iPhone 5S"]){
            bottomCell2_5s *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier2_5s forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            if (_detailArray.count > 0) {
                dayModel * model = _detailArray[indexPath.row];
                if([model.ifpass isEqual:@"0"])
                {
                ...
                int tempTime=[model.time intValue];
                NSDate *temp = [NSDate dateWithTimeIntervalSince1970:tempTime];
                cell.time.text =[dateFormatter stringFromDate:temp];
                cell.title.text = model.title;
                cell.leftNum.text = model.interval;
            ...
            bottomCell2 *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier2 forIndexPath:indexPath];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            if (_detailArray.count > 0) {
                dayModel * model = _detailArray[indexPath.row];
                if([model.ifpass isEqual:@"0"])
...
- (void)deleteAccountDetailWithID:(NSString *)category title:(NSString*)title time_data:(NSString*)time_data ID:(int)ID
{
    ...
    if (sqlite3_open([[self dataFilePath] UTF8String], &database)
        ...
        char *errmsg;
        NSString *deleteRow = [[NSString alloc]initWithFormat:@"DELETE FROM day WHERE category = '%@' and title='%@' and id='%d'" ,category,title,ID];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section == 0) {
        if([[AppDelegate deviceVersion] isEqual:@"iPhone 5S"]){
            ...
            if(_detailArray1.count >0) {
                dayModel * model = _detailArray1[0];
                if([model.ifpass isEqual:@"0"])
                {
                ...
                _cellForHeader_5s.time.text = model.interval;
            ...
            if(_detailArray1.count >0) {
                ...
                _cellForHeader.time.text = model.interval;
                headerView.backgroundColor=[UIColor clearColor];
